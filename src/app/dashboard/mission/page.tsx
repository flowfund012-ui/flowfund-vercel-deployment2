...
export default function MissionPage(){
  ...
  useEffect(()=>{
    ...
    sb.auth.getSession().then(async({data:{session}})=>{
      ...
      const[{data:tx},{data:b}]=await Promise.all([
      ...
      setTxns(tx??[]);setBudgets(b??[]);setLoading(false);
    });
  },[]);
  const now=new Date();
  ...
  const delTx=async(id:string)=>{setTxns(p=>p.filter(tx=>tx.id!==id));await sb.from('transactions').delete().eq('id',id);};
  ...
  const filtered=txns.filter(tx=>{if(filter!=='all'&&tx.type!==filter)return false;if(search&&!tx.description.toLowerCase().includes(search.toLowerCase())&&!tx.category.toLowerCase().includes(search.toLowerCase()))return false;return true;});
  ...
  return(
    <div style={{paddingBottom:48}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24}}>
        <div>
          <h1 style={{fontFamily:"'Orbitron',monospace",fontSize:22,fontWeight:700,color:'#00f2ff',marginBottom:4}}>{t(lang,'mission')}</h1>
          <p style={{fontSize:12,color:'rgba(255,255,255,.35)'}}>{t(lang,'track_every_dollar')} {now.toLocaleDateString('en-US',{month:'long',year:'numeric'})}</p>
        </div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setShowBudget(true)} style={{padding:'9px 14px',borderRadius:9,background:'rgba(0,242,255,.08)',border:'1px solid rgba(0,242,255,.2)',color:'#00f2ff',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:"'Inter',sans-serif"}}>+ {t(lang,'budget')}</button>
          <button onClick={()=>setShowAdd(true)} style={{padding:'9px 18px',borderRadius:9,background:'linear-gradient(135deg,#1a6bff,#7c00ff)',color:'#fff',border:'none',cursor:'pointer',fontSize:13,fontWeight:700,fontFamily:"'Inter',sans-serif",boxShadow:'0 4px 14px rgba(26,107,255,.3)'}}>+ {t(lang,'transaction')}</button>
        </div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:20}}>
        {[{l:t(lang,'total_income'),v:f(income),c:'#10b981',bg:'rgba(16,185,129,.07)',b:'rgba(16,185,129,.18)'},{l:t(lang,'total_expenses'),v:f(expenses),c:'#ef4444',bg:'rgba(239,68,68,.07)',b:'rgba(239,68,68,.18)'},{l:t(lang,'net_flow'),v:f(net),c:net>=0?'#10b981':'#ef4444',bg:net>=0?'rgba(16,185,129,.07)':'rgba(239,68,68,.07)',b:net>=0?'rgba(16,185,129,.18)':'rgba(239,68,68,.18)'}].map(c=>(
      ...
      {budgets.length>0&&(
        <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:20}}>
          <div style={{fontSize:10,color:'rgba(255,255,255,.35)',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:14,fontFamily:"'Orbitron',monospace"}}>{t(lang,'budget_vs_actual')}</div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:12}}>
            {budgets.map(b=>{const sp=cs[b.category]||0;const pct=Math.min(100,Math.round((sp/Number(b.amount))*100));const ov=pct>=100;return(
              <div key={b.id} style={{background:ov?'rgba(239,68,68,.06)':'rgba(255,255,255,.03)',border:`1px solid ${ov?'rgba(239,68,68,.2)':'rgba(255,255,255,.07)'}`,borderRadius:10,padding:'12px 14px'}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:7}}><span style={{fontSize:12,color:'rgba(255,255,255,.6)'}}>{b.category}</span><span style={{fontSize:11,color:ov?'#ef4444':'rgba(255,255,255,.35)'}}>{pct}%</span></div>
      ...
      {topCat.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:20}}>
          <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20}}>
            <div style={{fontSize:10,color:'rgba(255,255,255,.35)',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:14,fontFamily:"'Orbitron',monospace"}}>{t(lang,'top_spending')}</div>
            {topCat.map(([cat,amt])=>(
          ...
          <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(0,242,255,.15)',borderRadius:14,padding:20}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
              <div style={{fontSize:10,color:'#00f2ff',textTransform:'uppercase',letterSpacing:'.1em',fontFamily:"'Orbitron',monospace"}}>{t(lang,'ai_spending_tip')}</div>
              <button onClick={getAITip} disabled={aiLoading} style={{padding:'5px 12px',borderRadius:7,background:aiLoading?'rgba(255,255,255,.05)':'rgba(26,107,255,.15)',border:'1px solid rgba(26,107,255,.3)',color:'#60a5fa',cursor:aiLoading?'not-allowed':'pointer',fontSize:11,fontFamily:"'Inter',sans-serif"}}>
                {aiLoading?'...':t(lang,'get_tip')}
              </button>
            </div>
            {aiTip?<p style={{fontSize:13,color:'rgba(255,255,255,.7)',lineHeight:1.65}}>{aiTip}</p>
            :<p style={{fontSize:12,color:'rgba(255,255,255,.3)',lineHeight:1.6}}>{t(lang,'click_get_tip')}</p>}
          </div>
        </div>
      )}
      <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20}}>
        <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t(lang,'search_transactions')} style={{...inp,flex:1,minWidth:180,padding:'8px 14px'}}/>
          <div style={{display:'flex',gap:6}}>
            {(['all','income','expense'] as const).map(fv=>(
              <button key={fv} onClick={()=>setFilter(fv)} style={{padding:'7px 14px',borderRadius:8,fontSize:12,border:'1px solid',borderColor:filter===fv?'#1a6bff':'rgba(255,255,255,.1)',background:filter===fv?'rgba(26,107,255,.15)':'transparent',color:filter===fv?'#60a5fa':'rgba(255,255,255,.45)',cursor:'pointer',fontFamily:"'Inter',sans-serif",textTransform:'capitalize'}}>
                {fv==='all'?t(lang,'filter_all'):fv==='income'?t(lang,'total_income'):t(lang,'total_expenses')}
              </button>
            ))}
          </div>
        </div>
        {filtered.length===0?<div style={{textAlign:'center',padding:'32px 0',color:'rgba(255,255,255,.3)',fontSize:13}}>{t(lang,'no_transactions')}</div>:filtered.slice(0,50).map((tx:any)=>(
          <div key={tx.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 0',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:38,height:38,borderRadius:10,background:tx.type==='income'?'rgba(16,185,129,.1)':'rgba(239,68,68,.09)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:tx.type==='income'?'#10b981':'#ef4444',fontFamily:"'Roboto Mono',monospace",flexShrink:0}}>{tx.type==='income'?'+':'-'}</div>
              <div>
                <div style={{fontSize:13,color:'#e2e8f0',marginBottom:1}}>{tx.description}</div>
                <div style={{fontSize:11,color:'rgba(255,255,255,.28)'}}>{tx.category} · {new Date(tx.date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</div>
              </div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              ...
              <button onClick={()=>delTx(tx.id)} style={{background:'none',border:'none',color:'rgba(239,68,68,.35)',cursor:'pointer',fontSize:16,padding:0,lineHeight:1}}>×</button>
            </div>
          </div>
        ))}
      </div>
          ...
          <div style={{background:'#0d1117',border:'1px solid rgba(255,255,255,.1)',borderRadius:18,padding:28,width:'100%',maxWidth:440,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:14}}>
              {(['income','expense'] as const).map(txT=>(
                <button key={txT} onClick={()=>setForm(p=>({...p,type:txT,cat:txT==='income'?'Salary':'Food'}))} style={{flex:1,padding:'10px',borderRadius:9,border:'1px solid',borderColor:form.type===txT?(txT==='income'?'#10b981':'#ef4444'):'rgba(255,255,255,.1)',background:form.type===txT?(txT==='income'?'rgba(16,185,129,.12)':'rgba(239,68,68,.12)'):'transparent',color:form.type===txT?(txT==='income'?'#10b981':'#ef4444'):'rgba(255,255,255,.4)',cursor:'pointer',fontSize:13,fontWeight:700,fontFamily:"'Inter',sans-serif"}}>
                  {txT==='income'?t(lang,'income_btn'):t(lang,'expense_btn')}
                </button>
              ))}
            </div>
            {[{l:t(lang,'description'),k:'desc',p:t(lang,'desc_placeholder')},{l:t(lang,'amount'),k:'amount',p:'0.00',tp:'number'},{l:t(lang,'date'),k:'date',p:'',tp:'date'}].map(field=>(
              <div key={field.k} style={{marginBottom:12}}>
                ...
                <input type={field.tp||'text'} value={(form as any)[field.k]} onChange={e=>setForm(p=>({...p,[field.k]:e.target.value}))} placeholder={field.p} style={inp}/>
              </div>
            ))}
            <div style={{marginBottom:14}}>
              <label style={{display:'block',fontSize:11,color:'rgba(255,255,255,.38)',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em'}}>{t(lang,'category')}</label>
            ...
            <div style={{marginBottom:20}}>
              <label style={{display:'block',fontSize:11,color:'rgba(255,255,255,.38)',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em'}}>{t(lang,'notes_optional')}</label>
              <input value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} placeholder={t(lang,'optional_notes')} style={inp}/>
            </div>
            <button onClick={addTx} disabled={!form.desc.trim()||!form.amount||saving} style={{width:'100%',padding:'12px',borderRadius:10,background:'linear-gradient(135deg,#1a6bff,#7c00ff)',color:'#fff',border:'none',cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:"'Inter',sans-serif",opacity:saving?.6:1}}>
              {saving?t(lang,'saving'):t(lang,'add_transaction')}
            </button>
          </div>
        </div>
      )}
      {showBudget&&(
        <div onClick={e=>{if(e.target===e.currentTarget)setShowBudget(false)}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.78)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20,backdropFilter:'blur(4px)'}}>
          <div style={{background:'#0d1117',border:'1px solid rgba(0,242,255,.2)',borderRadius:18,padding:28,width:'100%',maxWidth:380}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
              <span style={{fontFamily:"'Orbitron',monospace",fontSize:13,color:'#00f2ff'}}>{t(lang,'set_budget')}</span>
            ...
            <div style={{marginBottom:22}}>
              <label style={{display:'block',fontSize:11,color:'rgba(255,255,255,.38)',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em'}}>{t(lang,'monthly_budget')}</label>
            ...
            <button onClick={addBudget} disabled={!budForm.amount||saving} style={{width:'100%',padding:'12px',borderRadius:10,background:'linear-gradient(135deg,#00b4d8,#0077b6)',color:'#fff',border:'none',cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:"'Inter',sans-serif",opacity:saving?.6:1}}>
              {saving?t(lang,'saving'):t(lang,'set_budget')}
            </button>
          </div>
        </div>
      )}
