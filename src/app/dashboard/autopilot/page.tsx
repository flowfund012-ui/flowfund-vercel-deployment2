...
export default function AutopilotPage() {
  ...
  if(loading)return <div style={{padding:40,textAlign:'center',color:'rgba(255,255,255,.4)'}}>{t(lang,'loading')} AutoPilot...</div>;
  ...
  return (
    <div style={{padding:'0 0 48px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24}}>
        <div>
          <h1 style={{fontFamily:"'Orbitron',sans-serif",fontSize:22,fontWeight:700,color:'#a78bfa',marginBottom:4}}>{t(lang,'autopilot')}</h1>
          <p style={{fontSize:12,color:'rgba(255,255,255,.35)'}}>{t(lang,'autopilot_subtitle')}</p>
        </div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setShowRule(true)} style={{padding:'8px 14px',borderRadius:8,background:'rgba(167,139,250,.1)',border:'1px solid rgba(167,139,250,.25)',color:'#a78bfa',cursor:'pointer',fontSize:12,fontWeight:600}}>+ {t(lang,'rule')}</button>
          <button onClick={()=>setShowGoal(true)} style={{padding:'8px 16px',borderRadius:8,background:'linear-gradient(135deg,#7c00ff,#1a6bff)',color:'#fff',border:'none',cursor:'pointer',fontSize:12,fontWeight:700}}>+ {t(lang,'goal')}</button>
        </div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:24}}>
        {[{l:t(lang,'active_goals'),v:String(activeGoals.length),c:'#a78bfa'},{l:t(lang,'total_saved'),v:f(totalSaved),c:'#10b981'},{l:t(lang,'total_target'),v:f(totalTarget),c:'#f59e0b'}].map(s=>(
      ...
      <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:20}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <span style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,color:'#a78bfa'}}>{t(lang,'smart_save_settings')}</span>
          <button onClick={saveSettings} style={{padding:'5px 12px',borderRadius:6,background:'rgba(167,139,250,.15)',border:'1px solid rgba(167,139,250,.25)',color:'#a78bfa',cursor:'pointer',fontSize:11}}>{t(lang,'save_settings')}</button>
        </div>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
          <div><div style={{fontSize:13,color:'#e2e8f0'}}>{t(lang,'auto_save_on_income')}</div><div style={{fontSize:11,color:'rgba(255,255,255,.3)'}}>{t(lang,'auto_save_desc')}</div></div>
        ...
        {smartSave&&(
          <div>
            <label style={{fontSize:12,color:'rgba(255,255,255,.4)',display:'block',marginBottom:6}}>{t(lang,'save_pct_label')} {smartPct}%</label>
            <input type="range" min={1} max={50} value={smartPct} onChange={e=>setSmartPct(+e.target.value)} style={{width:'100%',accentColor:'#a78bfa'}}/>
          </div>
        )}
      </div>
      <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:20}}>
        <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,color:'#a78bfa',marginBottom:16}}>{t(lang,'savings_goals')}</div>
        {activeGoals.length===0&&<div style={{textAlign:'center',padding:'20px 0',color:'rgba(255,255,255,.3)',fontSize:13}}>{t(lang,'no_goals')}</div>}
            ...
            <div key={g.id} style={{marginBottom:20,padding:16,background:'rgba(124,58,237,.04)',border:'1px solid rgba(124,58,237,.15)',borderRadius:12}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                <div>
                  ...
                  <div style={{fontSize:11,color:'rgba(255,255,255,.35)'}}>{g.category}{g.deadline?` - ${t(lang,'due')} ${new Date(g.deadline).toLocaleDateString('en-US',{month:'short',year:'numeric'})}`:''}</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:15,fontWeight:700,color:'#a78bfa',fontFamily:"'Roboto Mono',monospace"}}>{f(Number(g.current_amount))}</div>
                  <div style={{fontSize:11,color:'rgba(255,255,255,.3)'}}>{t(lang,'of')} {f(Number(g.target_amount))}</div>
                </div>
              </div>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <span style={{fontSize:11,color:pct>=80?'#10b981':'rgba(255,255,255,.3)'}}>{pct}% {t(lang,'complete')} - {f(left)} {t(lang,'to_go')}</span>
                <div style={{display:'flex',gap:6}}>
                  <input placeholder={t(lang,'add_amount')} value={addAmt} onChange={e=>setAddAmt(e.target.value)} style={{width:70,background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',borderRadius:6,padding:'4px 8px',color:'#fff',fontSize:11,outline:'none'}}/>
                  ...
                  <button onClick={()=>delGoal(g.id)} style={{padding:'4px 8px',borderRadius:6,background:'none',border:'none',color:'rgba(255,82,82,.4)',cursor:'pointer',fontSize:14}}>×</button>
                </div>
              </div>
            </div>
          );
        ...
        {completedGoals.length>0&&<div style={{marginTop:16}}>
          <div style={{fontSize:11,color:'rgba(255,255,255,.35)',marginBottom:8}}>{t(lang,'completed').toUpperCase()}</div>
          {completedGoals.map((g:any)=>(
            <div key={g.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
              ...
              <span style={{fontSize:12,color:'#10b981'}}>{t(lang,'complete')} - {f(Number(g.target_amount))}</span>
            </div>
          ))}
        </div>}
      </div>
      {rules.length>0&&(
        <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:20}}>
          <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,color:'#a78bfa',marginBottom:14}}>{t(lang,'automation_rules')}</div>
          {rules.map((r:any)=>(
            <div key={r.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
              <div>
                ...
                <div style={{fontSize:11,color:'rgba(255,255,255,.3)'}}>
                  {r.trigger_type==='on_income'?t(lang,'on_income_logged'):r.trigger_type==='monthly'?t(lang,'monthly'):t(lang,'weekly')} -
                  {r.action_type==='save_pct'?` ${t(lang,'save')} ${r.amount_pct}%`:r.action_type==='save_fixed'?` ${t(lang,'save')} ${f(r.amount_fixed)}`:` ${t(lang,'allocate_to_goal')}`}
                </div>
              </div>
      ...
      {log.length>0&&(
        <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20}}>
          <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,color:'#a78bfa',marginBottom:14}}>{t(lang,'automation_log')}</div>
          {log.map((l:any)=>(
        ...
        <div onClick={e=>{if(e.target===e.currentTarget)setShowGoal(false)}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}}>
          <div style={{background:'#0d1117',border:'1px solid rgba(167,139,250,.25)',borderRadius:16,padding:28,width:'100%',maxWidth:420}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <span style={{fontFamily:"'Orbitron',sans-serif",fontSize:13,color:'#a78bfa'}}>{t(lang,'new_goal')}</span>
              <button onClick={()=>setShowGoal(false)} style={{background:'none',border:'none',color:'rgba(255,255,255,.4)',cursor:'pointer',fontSize:20}}>×</button>
            </div>
            <div style={{marginBottom:10}}><input value={gName} onChange={e=>setGName(e.target.value)} placeholder={t(lang,'goal_name')} style={inp}/></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
              <input type="number" value={gTarget} onChange={e=>setGTarget(e.target.value)} placeholder={t(lang,'target_amount')} style={inp}/>
              <input type="number" value={gCurrent} onChange={e=>setGCurrent(e.target.value)} placeholder={t(lang,'current_amount')} style={inp}/>
            </div>
            ...
            <button onClick={addGoal} disabled={!gName.trim()||!gTarget||saving} style={{width:'100%',padding:'11px',borderRadius:8,background:'linear-gradient(135deg,#7c00ff,#1a6bff)',color:'#fff',border:'none',cursor:'pointer',fontSize:14,fontWeight:700,opacity:saving?.7:1}}>{saving?t(lang,'saving'):t(lang,'create_goal')}</button>
          </div>
        </div>
      )}
      {showRule&&(
        <div onClick={e=>{if(e.target===e.currentTarget)setShowRule(false)}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}}>
          <div style={{background:'#0d1117',border:'1px solid rgba(167,139,250,.25)',borderRadius:16,padding:28,width:'100%',maxWidth:420}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <span style={{fontFamily:"'Orbitron',sans-serif",fontSize:13,color:'#a78bfa'}}>{t(lang,'new_rule')}</span>
              <button onClick={()=>setShowRule(false)} style={{background:'none',border:'none',color:'rgba(255,255,255,.4)',cursor:'pointer',fontSize:20}}>×</button>
            </div>
            <div style={{marginBottom:10}}><input value={rName} onChange={e=>setRName(e.target.value)} placeholder={t(lang,'rule_name_placeholder')} style={inp}/></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
              <div>
                <label style={{fontSize:11,color:'rgba(255,255,255,.4)',display:'block',marginBottom:4}}>{t(lang,'trigger')}</label>
                <select value={rTrigger} onChange={e=>setRTrigger(e.target.value)} style={{...inp}}>
                  <option value="on_income" style={{background:'#0d1117'}}>{t(lang,'on_income_logged')}</option>
                  <option value="monthly" style={{background:'#0d1117'}}>{t(lang,'monthly')}</option>
                  <option value="weekly" style={{background:'#0d1117'}}>{t(lang,'weekly')}</option>
                </select>
              </div>
              <div>
                <label style={{fontSize:11,color:'rgba(255,255,255,.4)',display:'block',marginBottom:4}}>{t(lang,'action')}</label>
                <select value={rAction} onChange={e=>setRAction(e.target.value)} style={{...inp}}>
                  <option value="save_pct" style={{background:'#0d1117'}}>{t(lang,'save_pct_option')}</option>
                  <option value="save_fixed" style={{background:'#0d1117'}}>{t(lang,'save_fixed_option')}</option>
                  <option value="allocate_goal" style={{background:'#0d1117'}}>{t(lang,'allocate_to_goal')}</option>
                </select>
              </div>
            </div>
            {rAction==='save_pct'&&<div style={{marginBottom:10}}><input type="number" value={rPct} onChange={e=>setRPct(e.target.value)} placeholder={t(lang,'pct_to_save')} style={inp}/></div>}
            {rAction==='save_fixed'&&<div style={{marginBottom:10}}><input type="number" value={rFixed} onChange={e=>setRFixed(e.target.value)} placeholder={t(lang,'fixed_amount')} style={inp}/></div>}
            {rAction==='allocate_goal'&&<div style={{marginBottom:10}}>
              <select value={rGoal} onChange={e=>setRGoal(e.target.value)} style={{...inp}}>
                <option value="" style={{background:'#0d1117'}}>{t(lang,'select_goal')}</option>
                {activeGoals.map((g:any)=><option key={g.id} value={g.id} style={{background:'#0d1117'}}>{g.name}</option>)}
              </select>
            </div>}
            <button onClick={addRule} disabled={!rName.trim()||saving} style={{width:'100%',padding:'11px',borderRadius:8,background:'rgba(167,139,250,.2)',border:'1px solid rgba(167,139,250,.3)',color:'#a78bfa',cursor:'pointer',fontSize:14,fontWeight:700,marginTop:8}}>{saving?t(lang,'saving'):t(lang,'create_rule')}</button>
          </div>
        </div>
      )}
    </div>
