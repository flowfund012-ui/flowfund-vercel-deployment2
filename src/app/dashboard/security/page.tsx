...
export default function SecurityPage() {
  ...
  const exportCSV = async () => {
    ...
    const txCSV = [
      ...
      ...(txns||[]).map((tx:any)=>[tx.date,tx.type,tx.description,tx.category,tx.amount,tx.notes||''])
  ...
  if(loading)return <div style={{padding:40,textAlign:'center',color:'rgba(255,255,255,.4)'}}>{t(lang,'loading')} {t(lang,'security')}...</div>;
  return (
    <div style={{padding:'0 0 48px'}}>
      <h1 style={{fontFamily:"'Orbitron',sans-serif",fontSize:22,fontWeight:700,color:'#00ff9d',marginBottom:4}}>{t(lang,'security_title')}</h1>
      <p style={{fontSize:12,color:'rgba(255,255,255,.35)',marginBottom:28}}>{t(lang,'security_subtitle')}</p>
        ...
        <div style={{background:'rgba(0,255,157,.04)',border:'1px solid rgba(0,255,157,.15)',borderRadius:16,padding:22}}>
          ...
          {[
            ...
            <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'7px 0',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
              ...
              <span style={{fontSize:11,color:'#e2e8f0',fontFamily:"'Roboto Mono',monospace"}}>{v}</span>
            </div>
          ))}
        </div>
      ...
      <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:16,padding:22,marginBottom:20}}>
        ...
        <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
          ...
          <button onClick={signOut} style={{padding:'10px 20px',borderRadius:8,background:'rgba(255,82,82,.08)',border:'1px solid rgba(255,82,82,.2)',color:'#ff5252',cursor:'pointer',fontSize:13,fontWeight:600}}>{t(lang,'signout')}</button>
        </div>
      ...
      <div style={{background:'rgba(15,20,35,.7)',border:'1px solid rgba(255,255,255,.07)',borderRadius:16,padding:22}}>
        <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:12,color:'#00ff9d',marginBottom:16}}>{t(lang,'activity_log')} ({logs.length} events)</div>
        {logs.length===0?<div style={{textAlign:'center',padding:20,color:'rgba(255,255,255,.3)',fontSize:13}}>{t(lang,'no_activity')}</div>:
        logs.map((l:any)=>(
