...
const NAV=[
  ...
  {href:'/dashboard/debts',label:'Debt Planner',icon:'X',color:'#ef4444'},
...
export default function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  if(loading)return(
    <div style={{minHeight:'100vh',background:'#070810',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{textAlign:'center'}}><LogoMark size={56}/><div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:'rgba(255,255,255,.3)',letterSpacing:'.2em',marginTop:16}}>LOADING</div></div>
    </div>
  );
    ...
    <div style={{minHeight:'100vh',background:'#070810',color:'#f0f6ff',fontFamily:"'Inter',system-ui,sans-serif",display:'flex'}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav-item{transition:all .18s ease}.nav-item:hover{background:rgba(255,255,255,.055)!important;transform:translateX(2px)}
        ...
        @media(max-width:768px){main{margin-left:0!important;padding:68px 16px 32px!important}}
      `}</style>
      <aside className="sidebar-desk" style={{width:sidebarW,minHeight:'100vh',flexShrink:0,background:'rgba(7,8,16,.97)',borderRight:'1px solid rgba(255,255,255,.055)',position:'fixed',top:0,left:0,bottom:0,display:'flex',flexDirection:'column',zIndex:40,transition:'width .22s ease',overflowY:'auto',overflowX:'hidden'}}>
        <div style={{padding:'18px 14px 14px',borderBottom:'1px solid rgba(255,255,255,.055)',display:'flex',alignItems:'center',gap:10,justifyContent:collapsed?'center':'space-between'}}>
          {!collapsed?(
            ...
            <button onClick={()=>setCollapsed(false)} style={{background:'none',border:'none',cursor:'pointer',padding:0}}><LogoMark size={32}/></button>
          )}
        </div>
        {!collapsed&&profile&&(
          <div style={{padding:'10px 12px'}}>
            <div style={{padding:'8px 12px',borderRadius:10,background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.07)'}}>
              ...
              {score!==null&&(
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  <div style={{flex:1,background:'rgba(255,255,255,.07)',borderRadius:100,height:3}}>
                    <div style={{background:scoreColor(score),borderRadius:100,height:3,width:score+'%',transition:'width .5s'}}/>
                  </div>
                  <span style={{fontSize:10,fontWeight:700,color:scoreColor(score),fontFamily:"'Orbitron',monospace"}}>{score}</span>
                </div>
              )}
            </div>
          </div>
            ...
            return(
              <Link key={item.href} href={item.href} style={{textDecoration:'none'}}>
                <div className={`nav-item${active?' active':''}`} style={{display:'flex',alignItems:'center',gap:10,padding:collapsed?'10px 10px':'10px 12px',borderRadius:10,cursor:'pointer',position:'relative',background:active?`${item.color}10`:'transparent',border:'1px solid '+(active?`${item.color}28`:'transparent'),justifyContent:collapsed?'center':'flex-start'}}>
                  ...
                  <div style={{width:28,height:28,borderRadius:8,background:active?`${item.color}18`:'rgba(255,255,255,.05)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:active?item.color:'rgba(255,255,255,.38)',flexShrink:0,fontFamily:"'Orbitron',monospace",transition:'all .18s',border:active?`1px solid ${item.color}28`:'1px solid transparent'}}>{item.icon}</div>
      ...
      <div className="mob-bar" style={{position:'fixed',top:0,left:0,right:0,zIndex:50,height:56,background:'rgba(7,8,16,.97)',backdropFilter:'blur(20px)',borderBottom:'1px solid rgba(255,255,255,.055)',alignItems:'center',padding:'0 16px',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}><LogoMark size={28}/><span style={{fontFamily:"'Orbitron',monospace",fontSize:13,fontWeight:900,color:'#fff',letterSpacing:'.04em'}}>FlowFund</span></div>
        <button onClick={()=>setMobileOpen(!mobileOpen)} style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.1)',borderRadius:8,color:'rgba(255,255,255,.7)',cursor:'pointer',fontSize:16,padding:'6px 12px'}}>{mobileOpen?'X':'='}</button>
      </div>
      {mobileOpen&&(
