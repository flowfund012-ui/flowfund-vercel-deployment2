...
const NAV=[
  ...
  {href:'/dashboard/tax',label:'Tax Radar',color:'#f97316'},
  {href:'/dashboard/banksync',label:'Bank Sync',color:'#60a5fa'},
];
...
export default function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  if(loading)return(
    <div style={{minHeight:'100vh',background:'#070810',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{textAlign:'center'}}><WaveLogo size={52}/><div style={{fontFamily:"'Orbitron',monospace",fontSize:9,color:'rgba(255,255,255,.3)',letterSpacing:'.25em',marginTop:14}}>LOADING</div></div>
    </div>
  );
    ...
    <div style={{minHeight:'100vh',background:'#070810',color:'#f0f6ff',fontFamily:"'Inter',system-ui,sans-serif",display:'flex'}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:768px){.sd{display:none!important}.mb{display:flex!important}main{margin-left:0!important;padding:62px 14px 28px!important}}
        @media(min-width:769px){.mb{display:none!important}}
      `}</style>
      <aside className="sd" style={{width:sw,minHeight:'100vh',flexShrink:0,background:'rgba(6,7,14,.99)',borderRight:'1px solid rgba(255,255,255,.05)',position:'fixed',top:0,left:0,bottom:0,display:'flex',flexDirection:'column',zIndex:40,transition:'width .18s ease',overflowY:'auto',overflowX:'hidden'}}>
          ...
          {!collapsed?(
            <>
              <div style={{display:'flex',alignItems:'center',gap:9,minWidth:0}}>
                <WaveLogo size={28}/>
                <div><div style={{fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:900,color:'#fff',letterSpacing:'.04em',lineHeight:1.1}}>FlowFund</div><div style={{fontSize:7,color:'rgba(255,255,255,.22)',letterSpacing:'.22em',marginTop:2}}>FINANCIAL OS</div></div>
              </div>
              <button onClick={()=>setCollapsed(true)} style={{background:'none',border:'none',color:'rgba(255,255,255,.18)',cursor:'pointer',fontSize:13,padding:3,flexShrink:0}}>‹</button>
            </>
          ):(
            <button onClick={()=>setCollapsed(false)} style={{background:'none',border:'none',cursor:'pointer',padding:0}}><WaveLogo size={28}/></button>
          )}
        </div>
        {!collapsed&&profile&&(
          ...
          {NAV.map(item=>{
            ...
            return(
              <Link key={item.href} href={item.href} style={{textDecoration:'none'}}>
                <div className="ni" style={{display:'flex',alignItems:'center',gap:10,padding:collapsed?'10px 0':'9px 11px',borderRadius:7,cursor:'pointer',position:'relative',background:active?`${item.color}0c`:'transparent',justifyContent:collapsed?'center':'flex-start'}}>
                  {active&&!collapsed&&<div style={{position:'absolute',left:0,top:'18%',bottom:'18%',width:2.5,borderRadius:'0 2px 2px 0',background:item.color}}/>}
                  ...
                  {!collapsed&&<span style={{fontSize:12.5,fontWeight:active?600:400,color:active?item.color:'rgba(255,255,255,.45)',whiteSpace:'nowrap'}}>{item.label}</span>}
                </div>
              </Link>
            );
          })}
        ...
        <div style={{padding:'5px 8px',borderTop:'1px solid rgba(255,255,255,.05)'}}>
          <Link href="/dashboard/settings" style={{textDecoration:'none'}}>
            <div className="ni" style={{display:'flex',alignItems:'center',gap:10,padding:collapsed?'10px 0':'9px 11px',borderRadius:7,cursor:'pointer',justifyContent:collapsed?'center':'flex-start',marginBottom:1}}>
              ...
              {!collapsed&&<span style={{fontSize:12.5,color:'rgba(255,255,255,.38)'}}>Settings</span>}
            </div>
          </Link>
          <div className="ni" onClick={async()=>{await sb.auth.signOut();router.push('/login');}} style={{display:'flex',alignItems:'center',gap:10,padding:collapsed?'10px 0':'9px 11px',borderRadius:7,cursor:'pointer',justifyContent:collapsed?'center':'flex-start'}}>
            ...
            {!collapsed&&<span style={{fontSize:12.5,color:'rgba(239,68,68,.45)'}}>Sign Out</span>}
          </div>
        </div>
      </aside>
      <div className="mb" style={{position:'fixed',top:0,left:0,right:0,zIndex:50,height:54,background:'rgba(6,7,14,.99)',borderBottom:'1px solid rgba(255,255,255,.05)',alignItems:'center',padding:'0 14px',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:9}}><WaveLogo size={26}/><span style={{fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:900,color:'#fff'}}>FlowFund</span></div>
      ...
      {mobileOpen&&(
        <div style={{position:'fixed',inset:0,zIndex:49,background:'rgba(0,0,0,.72)'}} onClick={()=>setMobileOpen(false)}>
          <div style={{position:'absolute',top:54,left:0,right:0,background:'#090b18',borderBottom:'1px solid rgba(255,255,255,.06)',padding:8}} onClick={e=>e.stopPropagation()}>
            {NAV.map(item=>(
              <Link key={item.href} href={item.href} style={{textDecoration:'none'}} onClick={()=>setMobileOpen(false)}>
                <div style={{display:'flex',alignItems:'center',gap:9,padding:'10px 13px',borderRadius:6,background:isActive(item)?`${item.color}0c`:'transparent',marginBottom:1}}>
                  ...
                  <span style={{fontSize:13,color:isActive(item)?item.color:'rgba(255,255,255,.52)'}}>{item.label}</span>
                </div>
              </Link>
            ))}
          </div>
      ...
      <main style={{marginLeft:sw,flex:1,minHeight:'100vh',padding:'30px 26px',maxWidth:`calc(100vw - ${sw}px)`,transition:'margin-left .18s ease'}} className="ca">
