...
function Sidebar(){
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#070810',borderRight:'1px solid rgba(255,255,255,.06)',width:col?60:216,transition:'width .2s',overflow:'hidden'}}>
      <div style={{padding:'14px 10px',borderBottom:'1px solid rgba(255,255,255,.05)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <Logo full={!col}/>
        <button onClick={()=>setCol(c=>!c)} style={{background:'none',border:'none',color:'rgba(255,255,255,.25)',cursor:'pointer',fontSize:14,padding:'2px 4px'}}>{col?'›':'‹'}</button>
      </div>
      <nav style={{flex:1,padding:'8px 5px',overflowY:'auto'}}>
        {NAV.map(n=>{const act=path===n.href||(n.href!=='/dashboard'&&path?.startsWith(n.href));return(<Link key={n.href} href={n.href} title={col?n.label:''} style={{display:'flex',alignItems:'center',gap:col?0:9,padding:col?'10px':'9px 10px',borderRadius:9,marginBottom:1,background:act?'rgba(255,255,255,.06)':'transparent',color:act?'#fff':'rgba(255,255,255,.42)',textDecoration:'none',borderLeft:act?`2.5px solid ${n.c}`:'2.5px solid transparent',justifyContent:col?'center':'flex-start',transition:'all .15s'}}><span style={{fontSize:15,minWidth:18,textAlign:'center',flexShrink:0}}>{n.icon}</span>{!col&&<span style={{fontSize:12,fontWeight:act?600:400,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{n.label}</span>}</Link>);})}
      </nav>
...
export default function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return(
    <div style={{display:'flex',minHeight:'100vh',background:'#070810',fontFamily:"'Inter',sans-serif"}}>
      ...
      {!mob&&<div style={{flexShrink:0,position:'sticky',top:0,height:'100vh'}}><Suspense fallback={<div style={{width:216,background:'#070810',height:'100%'}}/>}><Sidebar/></Suspense></div>}
      {mob&&open&&<div style={{position:'fixed',inset:0,zIndex:200,display:'flex'}}><div style={{width:216,background:'#070810',height:'100%'}}><Suspense fallback={null}><Sidebar/></Suspense></div><div style={{flex:1,background:'rgba(0,0,0,.5)'}} onClick={()=>setOpen(false)}/></div>}
