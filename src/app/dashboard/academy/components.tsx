...
export const B:Record<string,any>={
  ...
  toast:{position:'fixed',bottom:24,left:'50%',transform:'translateX(-50%)',background:'#1a6bff',color:'#fff',padding:'12px 24px',borderRadius:10,fontWeight:600,fontSize:14,zIndex:9999,pointerEvents:'none',maxWidth:'90vw',textAlign:'center'},
...
export async function doUpload(file:File,folder:string,uid:string,onProgress?:(p:number)=>void):Promise<string|null>{
  return new Promise((resolve)=>{
    ...
    const url=`https://ammymxsyerlkdezsxuip.supabase.co/storage/v1/object/academy-uploads/${path}`;
    xhr.open('POST',url);
    xhr.setRequestHeader('Authorization','Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFtbXlteHN5ZXJsa2RlenN4dWlwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwOTI0NzMsImV4cCI6MjA4OTY2ODQ3M30.kS0xKDTl3KyjWBCB4Tp-8WdWPkAqXC62djKg4VPgC6E');
    xhr.setRequestHeader('x-upsert','true');
    xhr.setRequestHeader('Content-Type',file.type||'application/octet-stream');
    if(onProgress)xhr.upload.onprogress=e=>{if(e.lengthComputable)onProgress(Math.round((e.loaded/e.total)*100));};
    xhr.onload=()=>{
      if(xhr.status>=200&&xhr.status<300){resolve(STORAGE_URL+path);}
      else{console.error('Upload failed:',xhr.status,xhr.responseText);resolve(null);}
    };
    xhr.onerror=()=>{console.error('Upload error');resolve(null);};
    xhr.send(file);
  });
}

export function UploadField({label,accept,value,onUrl,uid,folder,type='file'}:{label:string,accept:string,value:string,onUrl:(u:string)=>void,uid:string,folder:string,type?:string}){
  const ref=useRef<HTMLInputElement>(null);
  const[prog,setProg]=useState(0);
  const[uploading,setUploading]=useState(false);
  const[err,setErr]=useState('');
  async function handle(f:File){
    setUploading(true);setProg(0);setErr('');
    const url=await doUpload(f,folder,uid,p=>setProg(p));
    setUploading(false);
    if(url){onUrl(url);setProg(100);}
    ...
    <div style={{marginBottom:14}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:6}}>
        ...
        <button type="button" disabled={uploading} onClick={()=>ref.current?.click()} style={{padding:'8px 16px',borderRadius:8,border:'1px dashed rgba(26,107,255,0.5)',background:'rgba(26,107,255,0.08)',color:'#1a6bff',cursor:uploading?'not-allowed':'pointer',fontSize:13,fontWeight:600,opacity:uploading?0.7:1,flexShrink:0}}>
          {uploading?`Uploading ${prog}%`:`📤 Upload ${type==='video'?'Video':type==='image'?'Image':'File'}`}
        </button>
        <span style={{color:'#475569',fontSize:12}}>or paste URL:</span>
        <input style={{...B.inp,flex:1,minWidth:160}} placeholder="https://..." value={value} onChange={e=>onUrl(e.target.value)} disabled={uploading}/>
      </div>
      {uploading&&<div style={{height:4,background:'rgba(255,255,255,0.08)',borderRadius:2,marginBottom:6}}><div style={{height:'100%',width:`${prog}%`,background:'linear-gradient(90deg,#1a6bff,#6c2ef5)',borderRadius:2,transition:'width 0.3s'}}/></div>}
      {err&&<div style={{fontSize:11,color:'#ef4444',marginBottom:4}}>{err}</div>}
      {value&&!uploading&&(
        type==='image'?<img src={value} alt="" style={{height:80,borderRadius:8,objectFit:'cover',marginTop:4}}/>
        :type==='video'&&value.startsWith('http')?<div style={{fontSize:11,color:'#10b981',marginTop:4}}>✓ Video ready: {value.split('/').pop()}</div>
        :null
      )}
    </div>
  );
      ...
      <div style={{maxWidth:760,margin:'0 auto',padding:'24px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:24,paddingBottom:20,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          ...
          <div style={{marginLeft:'auto',display:'flex',gap:16,alignItems:'center'}}>
            ...
            <button onClick={toggleLike} style={{background:'none',border:'none',cursor:user?'pointer':'default',display:'flex',alignItems:'center',gap:4,fontSize:13}}><span style={{fontSize:20,color:liked?'#ef4444':'#64748b'}}>{liked?'❤️':'🤍'}</span><span style={{color:'#64748b'}}>{likes}</span></button>
          </div>
        </div>
  ...
  return(
    <div style={{...B.wrap,display:'flex',flexDirection:'column',height:'100vh'}}>
      <div style={{background:'#0a1628',padding:'10px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
        ...
        <div style={{width:100,height:4,background:'rgba(255,255,255,0.08)',borderRadius:2,flexShrink:0}}><div style={{height:'100%',width:`${progress}%`,background:'linear-gradient(90deg,#1a6bff,#6c2ef5)',borderRadius:2,transition:'width 0.5s'}}/></div>
      </div>
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        <div style={{width:240,background:'rgba(255,255,255,0.02)',borderRight:'1px solid rgba(255,255,255,0.06)',overflowY:'auto',flexShrink:0}}>
          ...
          {lessons.map((l:any,i:number)=>{
            const done=progress>=(((i+1)/lessons.length)*100);const active=activeLesson?.id===l.id;
          ...
          {activeLesson?(
            <>
              ...
              {ltype(activeLesson)==='video'&&(()=>{const src=videoSrc(activeLesson);return(
                <div style={{width:'100%',aspectRatio:'16/9',background:'rgba(0,0,0,0.5)',borderRadius:12,overflow:'hidden',marginBottom:20,border:'1px solid rgba(255,255,255,0.08)'}}>
                  {src?(src.includes('youtube.com/embed')?<iframe width="100%" height="100%" src={src} style={{border:'none'}} allowFullScreen/>:<video src={src} controls style={{width:'100%',height:'100%',background:'#000'}} controlsList="nodownload"/>)
                  :(<div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'100%',color:'#475569'}}><div style={{fontSize:48}}>▶</div><div style={{fontSize:13,marginTop:8}}>No video attached</div></div>)}
                </div>
              );})()}
...
export function CreatorStudio({user,profile,onBack,showToast}:{user:any,profile:any,onBack:()=>void,showToast:(m:string)=>void}){
  ...
  async function loadDashboard(){
    ...
    if((courses||[]).length>0){
      ...
      const{count}=await sb.from('academy_lessons').select('id',{count:'exact',head:true}).in('course_id',ids);
      lessonCount=count||0;
    }
  ...
  async function saveProfile(){
    setSaving(true);
    const{error}=await sb.from('academy_creator_profiles').upsert({user_id:user.id,...ep,expertise:ep.expertise.split(',').map((s:string)=>s.trim()).filter(Boolean)},{onConflict:'user_id'});
    if(error)showToast('Error: '+error.message);else showToast('Profile saved!');
    setSaving(false);
  }

  async function saveCourse(publish:boolean){
    if(!nc.title.trim()){showToast('Course title is required');return;}
    setSaving(true);
    const payload={
      title:nc.title.trim(),description:nc.description,category:nc.category,level:nc.level,
      language:nc.language||'English',thumbnail_url:nc.thumbnail_url||null,
      price:parseFloat(nc.price)||0,is_free:parseFloat(nc.price)===0,
      what_you_learn:nc.what_you_learn.split('\n').filter((s:string)=>s.trim()),
      requirements:nc.requirements.split('\n').filter((s:string)=>s.trim()),
      instructor_id:user.id,
      instructor_name:ep.display_name||user.email?.split('@')[0]||'Instructor',
      is_published:publish,certificate_enabled:true,
    };
    ...
    if(courseId){
      ...
      if(error){showToast('Error creating: '+error.message);setSaving(false);return;}
      courseId=data.id;
    }
    // Save lessons
    ...
    if(validLessons.length>0){
      if(editingCourse){
        await sb.from('academy_lessons').delete().eq('course_id',courseId);
      }
      const rows=validLessons.map((l:any,i:number)=>({
        course_id:courseId,title:l.title.trim(),lesson_type:l.lesson_type||'text',
        content:l.content||'',video_url:l.video_url||null,order_index:i,
        is_free_preview:l.is_free_preview||false,questions:l.questions||null,
      }));
    ...
    setSaving(false);setTab('dashboard');setNc(blankCourse());setLs([{title:'Introduction',lesson_type:'text',content:'',video_url:'',is_free_preview:true}]);setEditingCourse(null);
    loadDashboard();
  }

  async function deleteCourse(id:string){
    if(!confirm('Delete this course and all its lessons?'))return;
    await sb.from('academy_lessons').delete().eq('course_id',id);
    await sb.from('academy_courses').delete().eq('id',id);
    showToast('Course deleted');loadDashboard();
  }

  async function togglePublish(course:any){
    await sb.from('academy_courses').update({is_published:!course.is_published}).eq('id',course.id);
    showToast(course.is_published?'Course unpublished':'Course published!');loadDashboard();
  }
  ...
  return(
    <div style={{...B.wrap,paddingBottom:80}}>
      <div style={{...B.hdr,flexWrap:'wrap',gap:8}}>
        <button style={{...B.back,padding:0}} onClick={isEditing?(()=>setTab('dashboard')):onBack}>← {isEditing?'Back to Dashboard':'Exit Studio'}</button>
        <h2 style={{...B.title,fontSize:18,margin:0}}>Creator Studio</h2>
        {!isEditing&&<div style={{marginLeft:'auto',display:'flex',gap:8}}>
          <button style={{...B.btn('primary'),padding:'7px 16px',fontSize:12}} onClick={()=>{setNc(blankCourse());setLs([{title:'Introduction',lesson_type:'text',content:'',video_url:'',is_free_preview:true}]);setEditingCourse(null);setTab('new-course');}}>+ New Course</button>
        </div>}
      </div>

      {!isEditing&&<div style={B.tabs}>
        {(['dashboard','profile']as const).map(t=><button key={t} style={B.tab(tab===t)} onClick={()=>setTab(t)}>{t==='dashboard'?'My Courses':'Profile'}</button>)}
      </div>}
      ...
      <div style={{padding:'20px 24px'}}>
        ...
        {tab==='profile'&&(
          <div style={{maxWidth:520}}>
            <h3 style={{color:'#e2e8f0',marginBottom:16,fontWeight:800}}>Creator Profile</h3>
            ...
            <div style={{marginBottom:20}}><label style={B.label}>Expertise (comma separated)</label><input style={B.inp} placeholder="e.g. Budgeting, Investing, Tax" value={ep.expertise} onChange={e=>setEp(p=>({...p,expertise:e.target.value}))}/></div>
        ...
        {tab==='dashboard'&&(
          <>
            ...
            <div style={{display:'flex',gap:12,marginBottom:20,flexWrap:'wrap'}}>
              {[['Courses',stats.courses,'#1a6bff'],['Total Students',stats.students,'#10b981'],['Lessons',stats.lessons,'#8b5cf6']].map(([l,v,c])=>(
                <div key={l as string} style={{background:'rgba(255,255,255,0.03)',border:`1px solid ${c}33`,borderRadius:12,padding:'14px 20px',minWidth:120,flex:1}}>
                  <div style={{fontSize:28,fontWeight:800,fontFamily:"'Orbitron',monospace",color:c as string}}>{v}</div>
            ...
            {/* Course list */}
            {myCourses.length===0?(
              <div style={{textAlign:'center',padding:40,color:'#475569',background:'rgba(255,255,255,0.02)',borderRadius:12,border:'1px dashed rgba(255,255,255,0.08)'}}>
                <div style={{fontSize:40,marginBottom:12}}>🎬</div>
                <div style={{marginBottom:16}}>No courses yet. Create your first course!</div>
                <button style={B.btn('primary')} onClick={()=>{setNc(blankCourse());setLs([{title:'Introduction',lesson_type:'text',content:'',video_url:'',is_free_preview:true}]);setEditingCourse(null);setTab('new-course');}}>Create Course</button>
              </div>
            ):(
              <div style={{display:'grid',gap:12}}>
                {myCourses.map((c:any)=>(
                  <div key={c.id} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:16,display:'flex',gap:14,alignItems:'center',flexWrap:'wrap'}}>
                    <div style={{width:48,height:48,borderRadius:10,background:c.thumbnail_url?'transparent':`${gc(c.category)}22`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0,overflow:'hidden'}}>
                      {c.thumbnail_url?<img src={c.thumbnail_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:gi(c.category)}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:700,color:'#e2e8f0',fontSize:14,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.title}</div>
                      <div style={{fontSize:12,color:'#64748b',marginTop:3,display:'flex',gap:10,flexWrap:'wrap'}}>
                        ...
                        <span>{c.level}</span>
                        <span style={{color:c.is_free?'#10b981':'#e2e8f0'}}>{c.is_free?'Free':`$${c.price}`}</span>
                        <span>👥 {c.total_enrollments||0} students</span>
                        {c.avg_rating>0&&<span style={{color:'#f59e0b'}}>★ {c.avg_rating}</span>}
                      </div>
                    </div>
        ...
        {isEditing&&(
          <div style={{maxWidth:760}}>
            <h3 style={{color:'#e2e8f0',marginBottom:20,fontWeight:800,fontSize:18}}>{editingCourse?'Edit Course':'Create New Course'}</h3>

            {/* Course thumbnail */}
            <UploadField label="Course Thumbnail" accept="image/jpeg,image/png,image/webp" value={nc.thumbnail_url||''} onUrl={u=>setNc((p:any)=>({...p,thumbnail_url:u}))} uid={user.id} folder="thumbnails" type="image"/>

            {/* Basic info */}
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div style={{gridColumn:'1/-1'}}><label style={B.label}>Course Title *</label><input style={B.inp} placeholder="e.g. Budgeting for Complete Beginners" value={nc.title} onChange={e=>setNc((p:any)=>({...p,title:e.target.value}))}/></div>
            ...
            <div style={{marginBottom:14}}><label style={B.label}>Description</label><textarea style={{...B.ta,minHeight:80}} placeholder="What is this course about? Who is it for?" value={nc.description} onChange={e=>setNc((p:any)=>({...p,description:e.target.value}))}/></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
              <div><label style={B.label}>What students will learn (one per line)</label><textarea style={B.ta} rows={5} placeholder={"Track monthly expenses\nBuild a savings habit\nUnderstand budgeting basics"} value={nc.what_you_learn} onChange={e=>setNc((p:any)=>({...p,what_you_learn:e.target.value}))}/></div>
              <div><label style={B.label}>Requirements (one per line)</label><textarea style={B.ta} rows={5} placeholder={"No experience needed\nA smartphone or computer"} value={nc.requirements} onChange={e=>setNc((p:any)=>({...p,requirements:e.target.value}))}/></div>
            </div>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                ...
                <div style={{display:'flex',gap:8}}>
                  {(['text','video','quiz']as const).map(t=>(
                    <button key={t} style={{...B.btn('ghost'),padding:'5px 12px',fontSize:11}} onClick={()=>setLs(p=>[...p,{title:'',lesson_type:t,content:'',video_url:'',is_free_preview:false}])}>
                      + {t==='video'?'Video':t==='quiz'?'Quiz':'Reading'}
                    </button>
                  ))}
                </div>
              </div>
              {ls.map((l:any,i:number)=>(
                <div key={i} style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:16,marginBottom:10}}>
                  <div style={{display:'flex',gap:8,marginBottom:10,alignItems:'center'}}>
                    <div style={{width:28,height:28,borderRadius:'50%',background:l.lesson_type==='video'?'rgba(26,107,255,0.2)':l.lesson_type==='quiz'?'rgba(139,92,246,0.2)':'rgba(16,185,129,0.2)',color:l.lesson_type==='video'?'#1a6bff':l.lesson_type==='quiz'?'#8b5cf6':'#10b981',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,flexShrink:0,fontWeight:700}}>
                      {l.lesson_type==='video'?'▶':l.lesson_type==='quiz'?'?':'T'}
                    </div>
                    ...
                    <select style={{...B.inp,width:'auto'}} value={l.lesson_type} onChange={e=>setLs(p=>p.map((x:any,j:number)=>j===i?{...x,lesson_type:e.target.value}:x))}>
                      <option value="text">📄 Reading</option><option value="video">▶ Video</option><option value="quiz">📝 Quiz</option>
                    </select>
                    <label style={{display:'flex',alignItems:'center',gap:5,fontSize:11,color:'#64748b',cursor:'pointer',flexShrink:0,whiteSpace:'nowrap'}}>
                      <input type="checkbox" checked={l.is_free_preview||false} onChange={e=>setLs(p=>p.map((x:any,j:number)=>j===i?{...x,is_free_preview:e.target.checked}:x))}/>Free
                    </label>
                    <button onClick={()=>setLs(p=>p.filter((_:any,j:number)=>j!==i))} style={{background:'none',border:'none',color:'#ef4444',cursor:'pointer',fontSize:16,flexShrink:0,padding:0}}>✕</button>
                  </div>
                  {l.lesson_type==='video'&&(
                    <UploadField label="" accept="video/mp4,video/webm,video/quicktime,video/x-m4v" value={l.video_url||''} onUrl={u=>setLs(p=>p.map((x:any,j:number)=>j===i?{...x,video_url:u}:x))} uid={user.id} folder="videos" type="video"/>
                  )}
                  <textarea style={{...B.ta,minHeight:l.lesson_type==='quiz'?52:80}} placeholder={l.lesson_type==='quiz'?'Quiz introduction / instructions (optional)':'Lesson notes, transcript, or reading material'} value={l.content||''} onChange={e=>setLs(p=>p.map((x:any,j:number)=>j===i?{...x,content:e.target.value}:x))}/>
                </div>
              ))}
            </div>
            ...
            <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
              <button style={{...B.btn('primary'),padding:'12px 28px'}} onClick={()=>saveCourse(true)} disabled={saving}>{saving?'Saving...':'Publish Course 🚀'}</button>
              <button style={{...B.btn('ghost'),padding:'12px 28px'}} onClick={()=>saveCourse(false)} disabled={saving}>{saving?'Saving...':'Save as Draft'}</button>
              <button style={{...B.btn('ghost'),padding:'12px 20px'}} onClick={()=>{setTab('dashboard');setEditingCourse(null);}}>Cancel</button>
            </div>
          </div>
        )}
      </div>
