...
const COUNTRIES=[
  ...
  {code:'CA',name:'Canada',cur:'CAD',sym:'$',vat:5,brackets:[{from:0,to:53359,rate:15},{from:53359,to:106717,rate:20.5},{from:106717,to:165430,rate:26},{from:165430,to:235675,rate:29.32},{from:235675,to:999999999,rate:33}]},
  {code:'AU',name:'Australia',cur:'AUD',sym:'$',vat:10,brackets:[{from:0,to:18200,rate:0},{from:18200,to:45000,rate:19},{from:45000,to:120000,rate:32.5},{from:120000,to:180000,rate:37},{from:180000,to:999999999,rate:45}]},
  ...
  {code:'SA',name:'Saudi Arabia',cur:'SAR',sym:'﷼',vat:15,brackets:[{from:0,to:999999999,rate:0}]},
  {code:'AE',name:'UAE',cur:'AED',sym:'د.إ',vat:5,brackets:[{from:0,to:999999999,rate:0}]},
  ...
  {code:'EG',name:'Egypt',cur:'EGP',sym:'£',vat:14,brackets:[{from:0,to:40000,rate:0},{from:40000,to:55000,rate:10},{from:55000,to:70000,rate:15},{from:70000,to:200000,rate:20},{from:200000,to:400000,rate:22.5},{from:400000,to:999999999,rate:25}]},
...
function calcTax(income:number,brackets:{from:number,to:number,rate:number}[]){
  ...
  return{tax:Math.round(tax*100)/100,eff:income>0?Math.round((tax/income)*1000)/10:0};
}
const fm=(n:number,sym='')=>sym+(n>=1000?n.toLocaleString('en-US',{minimumFractionDigits:0,maximumFractionDigits:0}):n.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}));
export default function TaxRadarPage(){
  ...
  useEffect(()=>{
    sb.auth.getSession().then(async({data:{session}})=>{
      ...
      const[{data:prefs},{data:txns}]=await Promise.all([
        sb.from('user_preferences').select('country_code').eq('user_id',uid).single(),
      ...
      setTxIncome(Math.round(inc*100)/100);
      setTxExpenses(Math.round(exp*100)/100);
      setLoading(false);
    });
  },[]);
  const saveCountry=async()=>{
    ...
    if(session){
      ...
      await sb.from('user_preferences').upsert({user_id:session.user.id,country_code:countryCode,currency:c?.cur||'USD',updated_at:new Date().toISOString()},{onConflict:'user_id'});
  ...
  const netProfit=txIncome-txExpenses;
  ...
  return(
    <div style={{paddingBottom:48}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontFamily:"'Orbitron',monospace",fontSize:20,fontWeight:700,color:'#f97316',marginBottom:4,filter:'drop-shadow(0 0 8px rgba(249,115,22,.4))'}}>Tax Radar</h1>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <select value={countryCode} onChange={e=>setCountryCode(e.target.value)} style={{...inp,width:'auto',background:'rgba(7,8,16,.95)'}}>
            {[...COUNTRIES].sort((a,b)=>a.name.localeCompare(b.name)).map(c=><option key={c.code} value={c.code} style={{background:'#0d1117'}}>{c.name}</option>)}
          </select>
          <button onClick={saveCountry} disabled={saving} style={{padding:'10px 18px',borderRadius:8,background:'rgba(249,115,22,.15)',border:'1px solid rgba(249,115,22,.3)',color:'#f97316',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:"'Inter',sans-serif",whiteSpace:'nowrap'}}>
      ...
      <div style={{background:'rgba(13,17,23,.9)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:16}}>
        <div style={{display:'flex',gap:10,marginBottom:16}}>
          <button onClick={()=>setUseTransactions(true)} style={{flex:1,padding:'10px',borderRadius:8,background:useTransactions?'rgba(249,115,22,.12)':'rgba(255,255,255,.03)',border:`1px solid ${useTransactions?'rgba(249,115,22,.35)':'rgba(255,255,255,.07)'}`,color:useTransactions?'#f97316':'rgba(255,255,255,.45)',cursor:'pointer',fontSize:12,fontWeight:useTransactions?600:400,fontFamily:"'Inter',sans-serif",transition:'all .15s'}}>
            Auto-Calculate from My Transactions
          </button>
          <button onClick={()=>setUseTransactions(false)} style={{flex:1,padding:'10px',borderRadius:8,background:!useTransactions?'rgba(249,115,22,.12)':'rgba(255,255,255,.03)',border:`1px solid ${!useTransactions?'rgba(249,115,22,.35)':'rgba(255,255,255,.07)'}`,color:!useTransactions?'#f97316':'rgba(255,255,255,.45)',cursor:'pointer',fontSize:12,fontWeight:!useTransactions?600:400,fontFamily:"'Inter',sans-serif",transition:'all .15s'}}>
            Enter Income Manually
          </button>
        </div>
        {useTransactions?(
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
            {[{l:'Total Income',v:fm(txIncome,ct.sym),c:'#10b981',bg:'rgba(16,185,129,.06)',b:'rgba(16,185,129,.18)'},{l:'Total Expenses',v:fm(txExpenses,ct.sym),c:'#ef4444',bg:'rgba(239,68,68,.06)',b:'rgba(239,68,68,.18)'},{l:'Net Profit',v:fm(netProfit,ct.sym),c:netProfit>=0?'#10b981':'#ef4444',bg:'rgba(255,255,255,.03)',b:'rgba(255,255,255,.07)'}].map(x=>(
          ...
          <div>
            ...
            <input type="number" value={manualIncome} onChange={e=>setManualIncome(e.target.value)} placeholder={`Enter your annual income in ${ct.cur}`} style={inp}/>
          </div>
        )}
      </div>
      ...
      {income>0&&(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:16}}>
          {[
            {l:'Income Tax Owed',v:fm(tax,ct.sym),sub:`${eff}% effective rate`,c:'#ef4444',bg:'rgba(239,68,68,.07)',b:'rgba(239,68,68,.22)'},
            {l:'Effective Rate',v:`${eff}%`,sub:`on ${fm(income,ct.sym)} income`,c:'#f59e0b',bg:'rgba(245,158,11,.07)',b:'rgba(245,158,11,.22)'},
            {l:'Take-Home Pay',v:fm(take,ct.sym),sub:'after income tax',c:'#10b981',bg:'rgba(16,185,129,.07)',b:'rgba(16,185,129,.22)'},
          ].map(x=>(
      ...
      {ct.vat>0&&income>0&&(
        <div style={{background:'rgba(249,115,22,.05)',border:'1px solid rgba(249,115,22,.15)',borderRadius:10,padding:'10px 16px',marginBottom:16,fontSize:12,color:'rgba(249,115,22,.8)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span>VAT / Sales Tax in {ct.name}</span>
      ...
      <div style={{background:'rgba(13,17,23,.9)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:22}}>
        <div style={{fontFamily:"'Orbitron',monospace",fontSize:11,color:'rgba(255,255,255,.4)',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:14}}>{ct.name} — Income Tax Brackets</div>
        {ct.brackets.map((b,i)=>{
          const toStr=b.to>=999999999?'∞':b.to.toLocaleString();
          const isActive=income>b.from&&income<=b.to;
          const isBelow=income>b.to;
          return(
            <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 14px',borderRadius:8,background:isActive?'rgba(249,115,22,.08)':isBelow?'rgba(255,255,255,.025)':'rgba(255,255,255,.02)',border:`1px solid ${isActive?'rgba(249,115,22,.25)':'rgba(255,255,255,.04)'}`,marginBottom:5,transition:'all .2s'}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                {isActive&&<div style={{width:6,height:6,borderRadius:'50%',background:'#f97316',flexShrink:0}}/>}
                {isBelow&&<div style={{width:6,height:6,borderRadius:'50%',background:'rgba(16,185,129,.5)',flexShrink:0}}/>}
                {!isActive&&!isBelow&&<div style={{width:6,height:6,borderRadius:'50%',background:'rgba(255,255,255,.1)',flexShrink:0}}/>}
                <span style={{fontSize:12,color:isActive?'rgba(255,255,255,.8)':'rgba(255,255,255,.4)',fontFamily:"'Roboto Mono',monospace"}}>{ct.sym}{b.from.toLocaleString()} — {toStr==='∞'?toStr:ct.sym+toStr}</span>
              </div>
              <span style={{fontSize:13,fontWeight:700,color:b.rate===0?'#10b981':b.rate<20?'#f59e0b':b.rate<35?'#f97316':'#ef4444',fontFamily:"'Roboto Mono',monospace"}}>{b.rate}%{isActive?' ← you':''}</span>
            </div>
          );
        })}
        {income===0&&<p style={{textAlign:'center',padding:12,fontSize:12,color:'rgba(255,255,255,.25)'}}>Your current bracket will highlight once income is calculated above.</p>}
      </div>
    </div>
  );
}
