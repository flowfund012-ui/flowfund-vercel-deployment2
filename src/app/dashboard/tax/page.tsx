...
export default function TaxRadarPage(){
  ...
  useEffect(()=>{
    ...
    sb.auth.getSession().then(async({data:{session}})=>{
      ...
      const inc=(txns??[]).filter((tx:any)=>tx.type==='income').reduce((s:number,tx:any)=>s+Number(tx.amount),0);
      const exp=(txns??[]).filter((tx:any)=>tx.type==='expense').reduce((s:number,tx:any)=>s+Number(tx.amount),0);
  ...
  if(loading)return<div style={{padding:40,textAlign:'center',color:'rgba(255,255,255,.4)'}}>{t(lang,'loading')} {t(lang,'tax')}...</div>;
  return(
    <div style={{paddingBottom:48}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontFamily:"'Orbitron',monospace",fontSize:20,fontWeight:700,color:'#f97316',marginBottom:4}}>{t(lang,'tax_title')}</h1>
          <p style={{fontSize:12,color:'rgba(255,255,255,.32)'}}>{t(lang,'tax_subtitle')}</p>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
          ...
          <button onClick={saveCountry} disabled={saving} style={{padding:'10px 18px',borderRadius:8,background:saved?'rgba(16,185,129,.15)':'rgba(249,115,22,.12)',border:`1px solid ${saved?'rgba(16,185,129,.3)':'rgba(249,115,22,.28)'}`,color:saved?'#10b981':'#f97316',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:"'Inter',sans-serif",whiteSpace:'nowrap'}}>
            {saving?t(lang,'saving'):(saved?'✓ Saved':t(lang,'save_country'))}
          </button>
        </div>
      </div>
      <div style={{background:'rgba(13,17,23,.9)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:20,marginBottom:16}}>
        ...
        {useTransactions?(
          ...
          <div>
            <label style={{display:'block',fontSize:11,color:'rgba(255,255,255,.35)',marginBottom:6,textTransform:'uppercase',letterSpacing:'.06em'}}>{t(lang,'annual_income')} ({ct.cur})</label>
      ...
      {income>0&&(
        ...
        {ct.code==='SA'||ct.code==='AE'?(
          <div style={{background:'rgba(16,185,129,.07)',border:'1px solid rgba(16,185,129,.2)',borderRadius:14,padding:'18px 22px',marginBottom:16,display:'flex',alignItems:'center',gap:14}}>
            ...
            <div>
              <div style={{fontSize:15,fontWeight:700,color:'#10b981',marginBottom:3}}>{ct.name} {t(lang,'no_income_tax')}</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:16}}>
            {[
              {l:t(lang,'income_tax_owed'),v:`${ct.sym} ${fmn(tax)}`,sub:`on ${ct.sym} ${fmn(income)} income`,c:'#ef4444',bg:'rgba(239,68,68,.07)',b:'rgba(239,68,68,.22)'},
              {l:t(lang,'effective_rate'),v:`${eff}%`,sub:'of your total income',c:'#f59e0b',bg:'rgba(245,158,11,.07)',b:'rgba(245,158,11,.22)'},
              {l:t(lang,'take_home'),v:`${ct.sym} ${fmn(take)}`,sub:'after all income tax',c:'#10b981',bg:'rgba(16,185,129,.07)',b:'rgba(16,185,129,.22)'},
            ].map(x=>(
      ...
      <div style={{background:'rgba(13,17,23,.9)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:22}}>
        <div style={{fontFamily:"'Orbitron',monospace",fontSize:11,color:'rgba(255,255,255,.4)',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:14}}>{ct.name} — {t(lang,'tax_brackets')}</div>
        {ct.brackets.map((b,i)=>{
        ...
        {income===0&&<p style={{textAlign:'center',padding:12,fontSize:12,color:'rgba(255,255,255,.25)'}}>{t(lang,'enter_income_hint')}</p>}
      </div>
    </div>
  );
}
