...
function avalanche(debts:any[]):any[]{return [...debts].sort((a,b)=>Number(b.interest_rate)-Number(a.interest_rate));}
function snowball(debts:any[]):any[]{return [...debts].sort((a,b)=>Number(a.balance)-Number(b.balance));}
export default function DebtPage(){
  ...
  const delDebt=async(id:string)=>{setDebts(p=>p.filter(d=>d.id!==id));await sb.from('debts').delete().eq('id',id);};
  ...
  return(
    <div style={{paddingBottom:48}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24}}>
        <div>
          <h1 style={{fontFamily:"'Orbitron',monospace",fontSize:22,fontWeight:700,color:'#ef4444',marginBottom:4}}>{t(lang,'debts')}</h1>
          <p style={{fontSize:12,color:'rgba(255,255,255,.35)'}}>{t(lang,'debts_subtitle')}</p>
        </div>
        <button onClick={()=>setShowAdd(true)} style={{padding:'9px 18px',borderRadius:10,background:'linear-gradient(135deg,#ef4444,#dc2626)',color:'#fff',border:'none',cursor:'pointer',fontSize:13,fontWeight:700,fontFamily:"'Inter',sans-serif"}}>+ {t(lang,'add_debt')}</button>
      </div>
      {debts.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:20}}>
          {[{l:t(lang,'total_debt'),v:f(totalBalance),c:'#ef4444',bg:'rgba(239,68,68,.07)',border:'rgba(239,68,68,.18)'},{l:t(lang,'monthly_minimums'),v:f(totalMinPayment),c:'#f59e0b',bg:'rgba(245,158,11,.07)',border:'rgba(245,158,11,.18)'},{l:t(lang,'debts_tracked'),v:String(debts.length),c:'#60a5fa',bg:'rgba(96,165,250,.07)',border:'rgba(96,165,250,.18)'}].map(c=>(
      ...
      {debts.length>0&&(
        <div style={{background:'rgba(13,17,23,.9)',border:'1px solid rgba(239,68,68,.2)',borderRadius:14,padding:22,marginBottom:20}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16,flexWrap:'wrap',gap:12}}>
            <div style={{fontFamily:"'Orbitron',monospace",fontSize:12,color:'#ef4444'}}>{t(lang,'ai_payoff_strategy')}</div>
            <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <span style={{fontSize:11,color:'rgba(255,255,255,.4)'}}>{t(lang,'extra_per_month')}:</span>
              ...
              <button onClick={getAIPlan} disabled={aiLoading} style={{padding:'7px 16px',borderRadius:8,background:aiLoading?'rgba(255,255,255,.06)':'linear-gradient(135deg,#1a6bff,#7c00ff)',color:aiLoading?'rgba(255,255,255,.4)':'#fff',border:'none',cursor:aiLoading?'not-allowed':'pointer',fontSize:12,fontWeight:700,fontFamily:"'Inter',sans-serif"}}>
                {aiLoading?t(lang,'analyzing'):t(lang,'get_ai_plan')}
              </button>
            </div>
          </div>
          {!aiPlan&&!aiLoading&&<div style={{fontSize:13,color:'rgba(255,255,255,.3)',textAlign:'center',padding:'16px 0'}}>{t(lang,'debt_plan_hint')}</div>}
      ...
      {orderedDebts.length>0&&(
        <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:22,marginBottom:20}}>
          <div style={{fontSize:10,color:'rgba(255,255,255,.35)',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:16,fontFamily:"'Orbitron',monospace"}}>{t(lang,'debt_breakdown')} — {strategy.charAt(0).toUpperCase()+strategy.slice(1)} {t(lang,'order')}</div>
              ...
              <div key={d.id} style={{padding:'16px 0',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                  <div style={{display:'flex',alignItems:'center',gap:12}}>
                    {i===0&&<div style={{padding:'2px 8px',borderRadius:100,background:'rgba(239,68,68,.15)',border:'1px solid rgba(239,68,68,.3)',fontSize:10,color:'#ef4444',fontWeight:700,flexShrink:0}}>{t(lang,'attack')}</div>}
                    <div>
                      <div style={{fontSize:14,fontWeight:600,color:'#e2e8f0',marginBottom:2}}>{d.name}</div>
                      <div style={{fontSize:11,color:'rgba(255,255,255,.35)'}}>{d.debt_type.replace('_',' ')} • {Number(d.interest_rate).toFixed(1)}% APR{d.due_day?` • ${t(lang,'due')} ${t(lang,'day')} ${d.due_day}`:''}</div>
                    </div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:18,fontWeight:700,color:'#ef4444',fontFamily:"'Roboto Mono',monospace"}}>{f(Number(d.balance))}</div>
                    <div style={{fontSize:11,color:'rgba(255,255,255,.35)'}}>min {f(Number(d.minimum_payment))}/{t(lang,'mo')}</div>
                  </div>
                </div>
                {d.original_balance>0&&pctPaid>0&&(
                  <div style={{marginBottom:8}}>
                    ...
                    <div style={{fontSize:10,color:'rgba(255,255,255,.3)',marginTop:3}}>{pctPaid}% {t(lang,'paid_off')}</div>
                  </div>
                )}
                <div style={{display:'flex',gap:16,flexWrap:'wrap'}}>
                  <div style={{fontSize:11,color:'rgba(255,255,255,.4)'}}>{t(lang,'payoff')}: <span style={{color:payoff.months>=999?'#ef4444':'#60a5fa'}}>{payoff.months>=999?t(lang,'never_min_too_low'):`${payoff.months} ${t(lang,'months')}`}</span></div>
                  {payoff.interest<999999&&<div style={{fontSize:11,color:'rgba(255,255,255,.4)'}}>{t(lang,'interest')}: <span style={{color:'#f59e0b'}}>{f(payoff.interest)}</span></div>}
                  {payoff.total<999999&&<div style={{fontSize:11,color:'rgba(255,255,255,.4)'}}>{t(lang,'total_paid')}: <span style={{color:'rgba(255,255,255,.6)'}}>{f(payoff.total)}</span></div>}
                </div>
                <div style={{marginTop:8}}>
                  <button onClick={()=>delDebt(d.id)} style={{fontSize:11,color:'rgba(239,68,68,.45)',background:'none',border:'none',cursor:'pointer',padding:0,fontFamily:"'Inter',sans-serif"}}>{t(lang,'remove')}</button>
                </div>
              </div>
            );
          })}
      ...
      {debts.length===0&&(
        <div style={{background:'rgba(13,17,23,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:14,padding:'48px 24px',textAlign:'center'}}>
          <div style={{fontSize:36,marginBottom:12,color:'rgba(239,68,68,.4)'}}>0</div>
          <h3 style={{fontFamily:"'Orbitron',monospace",fontSize:15,color:'rgba(255,255,255,.5)',marginBottom:8}}>{t(lang,'no_debts_tracked')}</h3>
          <p style={{fontSize:13,color:'rgba(255,255,255,.3)',marginBottom:24}}>{t(lang,'no_debts_desc')}</p>
          <button onClick={()=>setShowAdd(true)} style={{padding:'11px 28px',borderRadius:10,background:'linear-gradient(135deg,#ef4444,#dc2626)',color:'#fff',border:'none',cursor:'pointer',fontSize:13,fontWeight:700,fontFamily:"'Inter',sans-serif"}}>+ {t(lang,'track_first_debt')}</button>
        </div>
      )}
      {showAdd&&(
        <div onClick={e=>{if(e.target===e.currentTarget)setShowAdd(false)}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20,backdropFilter:'blur(4px)'}}>
          <div style={{background:'#0d1117',border:'1px solid rgba(239,68,68,.2)',borderRadius:18,padding:28,width:'100%',maxWidth:440,maxHeight:'90vh',overflowY:'auto'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <span style={{fontFamily:"'Orbitron',monospace",fontSize:14,color:'#ef4444'}}>{t(lang,'add_debt')}</span>
              <button onClick={()=>setShowAdd(false)} style={{background:'none',border:'none',color:'rgba(255,255,255,.4)',cursor:'pointer',fontSize:22}}>×</button>
            </div>
            {[{l:t(lang,'debt_name'),k:'name',p:'e.g. Chase Credit Card'},{l:t(lang,'current_balance'),k:'balance',p:'e.g. 3500',tp:'number'},{l:t(lang,'interest_rate_apr'),k:'interest_rate',p:'e.g. 19.99',tp:'number'},{l:t(lang,'minimum_payment'),k:'minimum_payment',p:'e.g. 75',tp:'number'},{l:t(lang,'due_day'),k:'due_day',p:'e.g. 15',tp:'number'}].map(field=>(
            ...
            <div style={{marginBottom:20}}>
              <label style={{display:'block',fontSize:11,color:'rgba(255,255,255,.4)',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em'}}>{t(lang,'debt_type')}</label>
              <select value={form.debt_type} onChange={e=>setForm(p=>({...p,debt_type:e.target.value}))} style={{...inp,background:'rgba(13,17,23,.95)'}}>
                {DEBT_TYPES.map(dt=><option key={dt} value={dt} style={{background:'#0d1117'}}>{dt.replace('_',' ')}</option>)}
              </select>
            </div>
            <button onClick={addDebt} disabled={!form.name.trim()||!form.balance||saving} style={{width:'100%',padding:'12px',borderRadius:10,background:'linear-gradient(135deg,#ef4444,#dc2626)',color:'#fff',border:'none',cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:"'Inter',sans-serif",opacity:saving?.7:1}}>
              {saving?t(lang,'saving'):t(lang,'add_debt')}
            </button>
          </div>
        </div>
      )}
